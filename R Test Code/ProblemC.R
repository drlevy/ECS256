        ...
        else
        {
              ...
              else
              {
                for(z in 2:queue_size)
                {
                  if( (k - z) > 0 )
                  {
                    ...
                    pij[state_i,state_j] = sum_prob*i*(portion_complex^(z-1))*(1.0-portion_complex)
          ...
          else if( i + j + k < queue_size )
          {
            if( k == 0 )
            {
                ...
                {
                  state_j = GenerateStateIndex(i-1, j, k, queue_size)
                  pij[state_i,state_j] = sum_prob*i*service_rate
                }
            ...
            else if( (k > 0) && (i == servers)) # can't have a queue with servers available
            {
              ...
              {
                state_j = GenerateStateIndex(i, j, k-1, queue_size)
                pij[state_i,state_j] = sum_prob*(customer_leave_rate + (i*service_rate*(1.0-portion_complex)))
              }
              ...
              state_j = GenerateStateIndex(i-1, j+k, 0, queue_size)
              pij[state_i,state_j] = sum_prob*i*(portion_complex^k)
            }
          }
        }
      }
